...
// To do.... for mouse drag function 
GLenum mode = GL_RENDER;
int z_mousePos[2] = { 0, 0 };
bool z_hit = false;
point z_hitPos, z_dragForce;
GLdouble projMatrix[16], modelMatrix[16];

// Load pictures
void textureLoad(int i,char *filename)
{
  Pic* img;
  img = ppm_read(filename, NULL);
  glBindTexture(GL_TEXTURE_2D, jello.texture[i]);
...
void display()
{
  ...
  //Window coordinates to object coordinates, by transformation
  ...
  */
  ...
  if(!isTexture) 
  {
		glGenTextures(1, &jello.texture[0]);
    textureLoad(0,"haha.ppm");
	}
