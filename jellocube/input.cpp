...
point valueAssign(double x, double y, double z)
{
  point res;
  res.x = x;
  res.y = y;
...
void mouseMotionDrag(int x, int y)
{
  ...
  if(g_iLeftMouseButton)
  {
    ...
    double length;
	  ...
	  p1 = valueAssign(x, viewport[3] - y, 1.0);
	  p2 = valueAssign(z_mousePos[0], viewport[3] - z_mousePos[1], 1.0);

    //Map window coordinates to object coordinates
	  gluUnProject(p1.x, p1.y, p1.z,modelMatrix, projMatrix, viewport, &(p1.x), &(p1.y), &(p1.z));
	  gluUnProject(p2.x, p2.y, p2.z,modelMatrix, projMatrix, viewport, &(p2.x), &(p2.y), &(p2.z));

	  pDIFFERENCE(p1,p2, z_dragForce);
   // pNORMALIZE(z_dragForce);
  }
...
// Reference------http://www.cs.mun.ca/av/old/teaching/cg/notes/select.pdf
void mouseButton(int button, int state, int x, int y)
{
	switch (button)
  {
    case GLUT_LEFT_BUTTON:
	  ...
	  if (!g_iLeftMouseButton)
		 memset(&z_dragForce,0,sizeof(point)),z_hit = 0;
	  else
		  (z_mousePos[0] = x, z_mousePos[1] = y);
      break;
  ...
  gluPerspective(60.0f, 1.0 * viewport[2] / viewport[3], 0.01f, 1000.0f);
  //draw scene for picking and perform
  showCube(&jello);
  glPopMatrix();
  glFlush();
...
void processHits(GLint hits, GLuint buffer[])
{
  ...
  {  
	...
	if (z_hit)
  {
		printf("hit point num = %d\n", names);
		z_hitPos = valueAssign(names / 64, (names - names / 64 * 64) / 8, names % 8 );
	}	
}
